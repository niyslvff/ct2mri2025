...
class Train:
    def __init__(
        ...
        **kwargs,
        ...
        # image_size_2d = (self.image_size[2], self.image_size[1])
        image_size_2d = (self.image_size[1], self.image_size[2])
        self.image_size_2d = image_size_2d
        new_image_size = tuple(s // 2 for s in image_size_2d) if self.crop else image_size_2d   # # (168, 224) or (84, 112)
        crop_transform = Compose([
            torchvision.transforms.RandomCrop(new_image_size),
            # torchvision.transforms.Resize(image_size_2d),
        ])
        ...
        self.dataloader = DataLoader(
            ...
            shuffle=True,
            drop_last=True,
            # collate_fn=self.dataloader_collate_fn_2d if self.dims == 2 else self.crop_collate_fn_3d,
        )
    ...
    def choose_model(self):
        if self.model_name.lower() == "dit":
            ...
            if self.version.lower() == "v1":
                ...
                raise "wrong version."
            return UNet(
                in_channel=self.in_channels * self.vae_embedding_dims if self.double_channels else self.in_channels,
                out_channel=self.vae_embedding_dims if self.double_channels else self.in_channels,
                dims=self.dims,
        ...
        for epoch in range(self.current_epoch, self.epoch_num):
            with tqdm(total=self.dataset.__len__() - self.dataset.__len__() % self.batch_size) as tq:
                ...
                for batch in self.dataloader:
                    ...
                    transformed_ct_noised_image, ct_real_noise = gaussain_scheduler.forward_diffusion_sample(transformed_ct_tensors, t, device=self.device)
                    # 如果输入channel数为2则模型输入包括 (带噪声图, 条件图, 时间步）
                    # 如果输入channel数为1则模型输入包括 (带噪声图, 时间步)
                    ct_pred_noise = self.model(transformed_ct_noised_image, transformed_mr_tensors, t) if self.in_channels == 2 else self.model(transformed_ct_noised_image, t)
                    loss = self.compute_loss(ct_pred_noise, ct_real_noise)
...
class BaseCommand:
    ...
    python3 train_diff_mri2ct.py --organ=brain --epoch_num=8000 --batch_size=2 --version=v1 --crop=1 --in_channels=2 --loss_function=double --inner_channels=64 --use_vae=1 --vae_embedding_dims=2 --double_channels=1 --image_root=../crop/preprocess_globalNormAndEnContrast --dataset_name=Raw3dDiffusionMajiDataset --vae_weight_file=./vqvae/weight_file/brain/v1/best.pth
