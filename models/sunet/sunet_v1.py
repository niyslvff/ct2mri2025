    ...
    def __init__(
        ...
        for ind in range(num_mults):  # 0 1 2 3 4
            ...
            if not is_last:
                ...
                c_downs.append(ManualAddPadding(pre_channel, dim=dims))
    ...
    def forward(self, x, c, time):
        ...
        for layer, c_layer in zip(self.downs, self.c_downs):
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                c, recover_matrix = c_layer(c, recover_matrix, down_n)
                continue
            else:
                x = layer(x)
                c = c_layer(c)
